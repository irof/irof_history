...
public class setting_main extends ListActivity {
	...
	/*
	 * final int DEF_INFOMATION = 0; final int DEF_OPERATE = 1; final int
	 * DEF_UPDATE = 2; final int DEF_VERSION = 3; final int DEF_TWITPIC = 4;
	 * final int DEF_TWITTER = 5; final int DEF_FACEBOOK = 6;
	 */
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		/*
		 * case DEF_INFOMATION:
		 * infoDlg(m_r.getString(R.string.info_title),m_r.getString
		 * (R.string.info_desc)); break; case DEF_OPERATE:
		 * infoDlg(m_r.getString(
		 * R.string.operate_title),m_r.getString(R.string.operate_desc)); break;
		 */
			case DEF_UPDATE:
				infoDlg(m_r.getString(R.string.update_title),
						m_r.getString(R.string.update_desc));
				break;
			case DEF_VERSION:
				String appVer = "";
				try {
					Context context = getApplicationContext();
					PackageInfo m_sPackInfo = context.getPackageManager()
							.getPackageInfo(context.getPackageName(), 0);
					String verS = m_sPackInfo.versionName;
					appVer = String.format("%s/%s", verS,
							m_sPackInfo.versionCode);
				}
				catch (NameNotFoundException e) {
				}
				showMessageBoxWelcome(m_r.getString(R.string.app_name),
						"version: " + appVer);
				break;
			/*
			 * case DEF_TWITPIC: String inURL
			 * ="http://twitpic.com/tag/irofhistory"; Intent browse = new
			 * Intent( Intent.ACTION_VIEW , Uri.parse( inURL ) ); startActivity(
			 * browse ); break;
			 */
			case DEF_TWITTER:
				...
				Intent intentTw = new Intent(this,
			...
			case DEF_FACEBOOK:
				...
				Intent intentFb = new Intent(this,
	...
	public void showMessageBoxWelcome(final String title, final String message) {
		...
		if (isConnected()) {// なんらかの接続チェック情報
			...
			try {
				Context context = getApplicationContext();
				m_sPackInfo = context.getPackageManager().getPackageInfo(
			...
			String url = "https://androidquery.appspot.com/api/market?app="
					+ packageName + "&locale=" + Locale.getDefault();
				...
				public void onSuccess(JSONObject response) {
					try {
						...
						if (verS.compareTo(version) < 0) {
							...
							// version different
							alertDialogBuilder.setMessage(message
									+ m_r.getString(R.string.Message_Update,
											published, version));
							alertDialogBuilder.setNegativeButton("Update",
									new DialogInterface.OnClickListener() {
										...
										public void onClick(
												...
												int which) {
											String inURL = "market://details?id="
													+ packageName;
