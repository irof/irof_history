...
public abstract class commonPostTask extends AsyncTask<String, String, String> {
	...
	private String			TAG			= commonPostTask.this.getClass()
												.getSimpleName();
	...
	protected File saveImageGalleryFile() {
		...
		try {
			String ret = MediaStore.Images.Media.insertImage(
					activity.getContentResolver(),
					activity.getBitmap(activity.findViewById(R.id.root)), "",
					null);
			if (ret == null) return null;
			Cursor c = activity.getContentResolver().query(Uri.parse(ret),
					null, null, null, null);
			c.moveToFirst();
	...
	protected File saveImageFile() {
		...
		try {
			...
			FileOutputStream fo = activity.openFileOutput("dst.txt",
	...
	private String upload(String str, File newfile, MediaProvider mediaprovider)
		...
		ConfigurationBuilder cb2 = new ConfigurationBuilder()
				.setOAuthConsumerKey(
						twitter.getConfiguration().getOAuthConsumerKey())
				.setOAuthConsumerSecret(
						twitter.getConfiguration().getOAuthConsumerSecret())
		...
		// upload1
		twitter4j.media.ImageUpload upload1 = new ImageUploadFactory(conf)
				.getInstance(mediaprovider); // mediaproviderを指定する
