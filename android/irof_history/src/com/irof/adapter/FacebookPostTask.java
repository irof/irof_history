...
public class FacebookPostTask extends commonPostTask {
	...
	protected String doInBackground(String... params) {
		...
		try {
			...
			String photoId = facebook_main.m_facebook.postPhoto(source, str,
					"", true);// trueだとtimeline上は非表示
			//
			Photo photo = facebook_main.m_facebook.getPhoto(photoId);
			if (exURL == null) exURL = ShortURL.expand(new URL(m_r
					.getString(R.string.fb_login_url)));
			PostUpdate post = new PostUpdate(exURL).picture(photo.getPicture())
					.message(str);
