...
public class AuthTwActivity extends Activity implements AuthTwWebView.AuthCallback {
	...
	private void OAuth() {
		...
		try {
			...
			if (CALLBACK_URL != null) {
				...
				twitter_main.mOAuth.setOAuthConsumer(
						twitter_main.CONSUMER_KEY,
						twitter_main.CONSUMER_SECRET);

				try {
					twitter_main.m_requestToken = 
							twitter_main.mOAuth.getOAuthRequestToken(CALLBACK_URL);// "callbackttnlive://CallbackActivity");
				}
				catch (Exception e) {
			...
			if (twitter_main.m_requestToken == null) {
				...
				twitter_main.m_requestToken = twitter_main.m_twitter
